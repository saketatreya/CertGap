...
\caption{\textbf{The distribution mismatch decouples value-loss from improvement.} (Panel 1) The rollout distribution $d^\pi$ contains all states traversed in an episode, over which $\epsilon_u$ is optimized. However, actual policy improvement is evaluated strictly at the start states $\nu$. (Panel 2) As a consequence, the absolute Bellman residual $|\epsilon_u|$ is near-blind to ground-truth improvement $\Delta J_k$ (Humanoid-v5, representative run). (Panel 3) In contrast, the start-state bias $-\hat\Delta_k$ exposes the operative relationship. (Panel 4) Median per-seed AUROC for harm prediction: $-\epsilon_u$ (gray) sits at or below chance across PPO, SAC, and TRPO; $-\hat\Delta_k$ (colored) is reliably predictive in every algorithm. Full per-environment table is \Cref{tab:per-env-auroc}.}
...
\paragraph{Targeting the bias directly: a structural limit.}
If the boundary is merely an artifact of missing loss terms, targeting the bias directly in the critic loss should allow the critic to generalize prospectively. We evaluate a $\nu$-weighted critic loss (adding a start-state MSE penalty $L = \text{MSE}(V(s), G_s) + \lambda \cdot \text{MSE}(V(s_0), G_0)$) on Humanoid-v5, HalfCheetah-v5, and Hopper-v5 with $\lambda=1.0$. \Cref{fig:nu-weighted} shows this addition fails to drive down $|\hat\Delta_k|$ compared to vanilla PPO and leaves the lag-1 AUROC at chance (pooled 0.54 vs vanilla 0.56), while maintaining final-return parity. The diagnostic remains stubbornly retrospective even when explicitly targeted by the loss. We interpret this null result as a structural observation: the retrospective boundary is a persistent consequence of the optimization order, not merely a deficiency of the standard MSE loss.
...
\paragraph{Operational feasibility: post-hoc gate via checkpoint and rollback.}
