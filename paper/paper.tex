...
\begin{abstract}
Actor--critic deep reinforcement learning (RL) relies on learned value functions to guide policy updates. Practitioners typically monitor the Bellman residual ($\epsilon_u$), or value-function loss, as a proxy for critic quality and update health. We demonstrate, both theoretically and empirically, that across the actor--critic configurations we audit the absolute Bellman residual is uninformative as a per-update predictor of policy improvement. Theoretically, we show that the relevant criterion is a \emph{dimensionless ratio} $\epsilon_u / A_k$, and that any fixed absolute tolerance eventually becomes vacuous as the policy improves. Empirically, we audit over 86{,}000 actor--critic updates across PPO, SAC, and TRPO, spanning eight environments and 237 seeds. We find $\epsilon_u$ is near-chance at predicting policy harm (pooled median AUROC $0.46$). The exact policy-improvement identity $J(\pi_{k+1}) - J(\pi_k) = A_k - \hat\Delta_k$ exposes the \textbf{start-state critic bias} $\hat\Delta_k = J(\pi_k) - \mathbb{E}_\nu V_{k+1}$ as the operative covariate-shift quantity. The bias term predicts harm at pooled median AUROC $0.68$ and dominates $\epsilon_u$ on $234$ of $236$ paired seeds (paired Wilcoxon $p < 10^{-40}$). To rule out return-estimate leakage, we replicate the diagnostic on independent held-out rollouts and recover AUROC $0.61$, confirming the signal reflects critic generalization rather than shared Monte Carlo noise. The dominance is uniform across PPO, SAC, and TRPO, robust to a $\sim$200-cell hyperparameter factorial, and matches a tabular sanity check where the identity holds to machine precision. Finally, directly targeting the bias via a $\nu$-weighted critic loss leaves the lag-1 AUROC at chance, suggesting the boundary between retrospective diagnostic and prospective controller is fundamental to the optimization order. We conclude that critic monitoring in actor--critic deep RL should shift from absolute TD error toward relative, distribution-aware quantities.
\end{abstract}
...
Our contributions are: (i) a structural argument that absolute Bellman residuals cannot certify policy improvement, formalized as a no-go theorem (\Cref{prop:no-go}); (ii) identification of the start-state critic bias $\hat\Delta_k$ as the mechanistically active signal, via the exact identity $J(\pi_{k+1}) - J(\pi_k) = A_k - \hat\Delta_k$; (iii) a large-scale audit across PPO, SAC, and TRPO giving the first cross-algorithm empirical evidence that value-function loss is decoupled from policy improvement.
...
\paragraph{Targeting the bias directly: a $\nu$-weighted critic loss.}
Because the diagnostic identifies the start-state bias as the operative error, a natural intervention is to target it directly in the critic loss. We evaluate a $\nu$-weighted critic loss that adds a start-state MSE penalty: $L = \text{MSE}(V(s), G_s) + \lambda \cdot \text{MSE}(V(s_0), G_0)$, directly pushing $V_{k+1}(s_0)$ toward the episode return. \Cref{fig:nu-weighted} evaluates this method on Humanoid-v5, HalfCheetah-v5, and Hopper-v5 with $\lambda=1.0$. The addition fails to drive down $|\hat\Delta_k|$ compared to vanilla PPO and leaves the lag-1 AUROC at chance (pooled 0.54 vs vanilla 0.56), while maintaining final-return parity. The diagnostic remains stubbornly retrospective even when the loss explicitly targets it. We interpret this null result as a structural observation: the retrospective boundary is a fundamental consequence of the optimization order, not merely a deficiency of the standard MSE loss.
...
\paragraph{What practitioners should do today.}
The immediate prescription is to log $\widehat{\hat\Delta}_k$ alongside $\widehat\epsilon_u$ in any actor--critic codebase. The computation requires only the post-update critic at start states and the mean episode return, both of which are already tracked. Two concrete uses follow.

(1) \emph{Hyperparameter ranking.} On the LunarLander $4\times 4$ factorial, the seed-mean concurrent $-\hat\Delta_k$ correlates with final mean return at Spearman $\rho = 0.95$ across the 16 cells (\Cref{fig:hyperparam}). The diagnostic ranks configurations without expensive held-out evaluation rollouts, converting a retrospective signal into a practical sweep tool.

(2) \emph{Checkpoint-and-rollback gate.} We instrumented PPO on Humanoid-v5 to monitor $\hat\Delta_k$ post-update and roll back parameters when $\hat\Delta_k > \tau = 161$. Over 5 seeds at $500$k steps, the gate rejected $38.3\%$ of candidate updates. Cross-seed dispersion of the final return tightened: standard deviation $19.9 \to 15.3$ (a $23\%$ reduction); the across-seed standard deviation averaged over training updates fell from $21.5$ to $18.9$ ($12\%$). Mean final return ($449.2$) is statistically indistinguishable from vanilla ($463.1$; Welch $t \approx 1.2$, $n=5$ per arm). We do not claim the gate improves PPO. It does not eliminate catastrophic single-update drops either: the rate of updates with $\Delta J_k < -50$ is comparable for vanilla ($2.8\%$) and gated ($3.4\%$). This is an actual training experiment, not a retrospective audit (cf.\ \Cref{fig:mechanism-intervention}, right), and we present it as a feasibility demonstration: the diagnostic is actionable in real codebases and tightens the cross-seed distribution at no cost in mean return, but does not by itself constitute a state-of-the-art training algorithm.
...
The Bellman residual is the loss \emph{minimized} during training, not just the scalar practitioners watch. A different target is suggested by \Cref{prop:no-go}: drive down $\hat\Delta_k$ in a sense that satisfies the relative criterion $\epsilon_{u,k} \le c \cdot A_k$ as the policy improves. However, as demonstrated in \Cref{sec:intervention}, directly targeting $\hat\Delta_k$ via a $\nu$-weighted regularizer leaves the retrospective boundary intact. A successful objective may require tighter Lipschitz-control of $|\hat\Delta_k| \le L \cdot W_1(\nu, d^{\pi_k})$ via spectral normalisation \citep{miyato2018spectral,bjorck2021spectralnorm}. Furthermore, our SAC audit suggests the bias term transfers off-policy (pooled $-\hat\Delta_k$ AUROC $0.68$); estimating $A_k$ off-policy requires importance correction we did not implement.

\paragraph{Limitations.}
$\widehat{\hat\Delta}_k$ inherits the variance of $\widehat J(\pi_k)$ on environments with few complete episodes per rollout (HalfCheetah $\sim 2$, Ant $\sim 3$); the dominance over $\widehat\epsilon_u$ holds nevertheless. The lag-1 collapse means $\hat\Delta_k$ is a diagnostic, not an online controller; the rollback gate of \Cref{sec:intervention} mitigates but does not eliminate this. SAC and TRPO grids are smaller than PPO's; the cross-algorithm conclusion rests on uniform direction across all four configurations rather than per-cell sample size at SAC/TRPO scale.

\section{Conclusion}
\label{sec:conclusion}

Across PPO/SAC/TRPO ($237$ seeds, eight environments) the Bellman residual pools to AUROC $0.46$ as a harm predictor, while the identity $\Delta J_k = A_k - \hat\Delta_k$ exposes start-state bias as the operative quantity: $-\hat\Delta_k$ pools to $0.68$ and dominates $-\widehat\epsilon_u$ on $234/236$ paired seeds ($p < 10^{-40}$). Critic monitoring and design in actor--critic deep RL should target start-state bias, not the per-state residual.
