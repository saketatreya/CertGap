...
\caption{\textbf{The distribution mismatch decouples value-loss from improvement.} (Panel 1) The Bellman residual $\epsilon_u$ is minimized over the rollout distribution $d^\pi$ (all states traversed in an episode). However, the true objective---policy improvement---depends exclusively on critic accuracy at the start states $\nu$. (Panel 2) Because it measures the wrong distribution, the absolute Bellman residual $|\epsilon_u|$ is near-blind to ground-truth improvement $\Delta J_k$ (Humanoid-v5, representative run). (Panel 3) In contrast, measuring the critic's error exactly at the start states (the start-state bias $-\hat\Delta_k$) exposes the operative relationship. (Panel 4) Median per-seed AUROC for harm prediction: $-\epsilon_u$ (gray) sits at or below chance across PPO, SAC, and TRPO; $-\hat\Delta_k$ (colored) is reliably predictive in every algorithm. Full per-environment table is \Cref{tab:per-env-auroc}.}
...
\section{Empirical limits of prospective control}
\label{sec:intervention}

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{fig2_mechanism_intervention.pdf}
\caption{\textbf{The $A_k$ paradox: positive coupling with the bias makes the surrogate misleading.} (Left) Joint density of $A_k$ and $\Delta J_k$ on PPO Humanoid-v5: large $A_k$ frequently coincides with negative $\Delta J_k$. (Center) Joint density of $A_k$ and $\hat\Delta_k$: positive coupling ($\rho = 0.57$) explains the paradox. (Right) Evidence of operational relevance: sweeping a retrospective threshold on $\hat\Delta_k$ safely isolates harmful updates on Humanoid-v5.}
\label{fig:mechanism-intervention}
\end{figure}

The distribution mismatch forces a strict boundary on how the diagnostic can be used: it is a powerful retrospective auditor, but it cannot be easily converted into a prospective online controller. 

\paragraph{The retrospective boundary: lag-1 collapse.}
Because the diagnostic $\hat\Delta_k$ is computed using the post-update critic $V_{k+1}$, it is intrinsically retrospective. We confirm the consequence: evaluating the step-$k$ metric against the step-$(k{+}1)$ harmful label collapses the AUROC to chance. Pooled lag-1 AUROC across the PPO grid is $0.51$, with mild heterogeneity (Hopper $0.65$, Walker2d $0.57$, others within $\pm 0.05$ of $0.50$). The signal is real and concurrent, but it does not look one step ahead. 

\paragraph{Targeting the bias directly: evidence of a structural boundary.}
A natural question is whether we can simply solve the problem by penalizing the start-state bias in the critic loss. We evaluate a $\nu$-weighted critic loss (adding a start-state MSE penalty $L = \text{MSE}(V(s), G_s) + \lambda \cdot \text{MSE}(V(s_0), G_0)$) on Humanoid-v5, HalfCheetah-v5, and Hopper-v5 with $\lambda=1.0$. \Cref{fig:nu-weighted} shows this addition completely fails to drive down $|\hat\Delta_k|$ compared to vanilla PPO and leaves the lag-1 prospective AUROC at chance (pooled 0.54 vs vanilla 0.56), while maintaining final-return parity. The diagnostic remains stubbornly retrospective even when explicitly targeted by the loss. We interpret this null result as powerful evidence of a structural boundary: the problem is not a deficiency of the standard MSE loss, but a persistent consequence of the actor-critic optimization order itself. We cannot simply add a loss term to fix it.
...
While standard optimization cannot prospectively fix the bias, practitioners can still embrace its retrospective nature operationally. We evaluate a simple checkpoint-and-rollback pattern: compute $V_{k+1}$ on a candidate update, evaluate $\hat\Delta_k$, and roll back if $\hat\Delta_k > \tau$. Sweeping $\tau$ on the PPO Humanoid audit (\Cref{fig:mechanism-intervention}, right) shows a threshold flagging the worst-$\hat\Delta_k$ updates retains $\sim 80\%$ of the cumulative improvement while removing $\sim 60\%$ of the harmful updates by count. We validate this in actual training in \Cref{sec:discussion}, where an active rollback gate tightens the variance of training trajectories without improving mean return. We present this strictly as evidence that the diagnostic has operational value, not as a proposed control algorithm or a new state-of-the-art.

\paragraph{Robustness and Sanity Checks.}
To ensure these findings are not artifacts of specific implementation details, we provide extensive robustness checks in the Appendix. We verify the start-state bias signal survives on fully independent held-out rollouts to rule out Monte Carlo leakage (\Cref{app:agreement}). We show the dominance holds across a 200-cell hyperparameter grid of value-epochs and clip thresholds (\Cref{fig:factorial}), and that the failure of $\widehat\epsilon_u$ persists across alternative scalarizations like the sup-norm and 95th-percentile (\Cref{app:eps-u-variants}). Finally, we confirm the algebraic identity holds to machine precision in finite-state RandomMDPs (\Cref{app:tabular}) and that non-linear Random Forest combinations do not improve upon the additive identity.
